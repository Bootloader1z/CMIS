...
class DashboardController extends Controller
{
    public function getViolations(Request $request)
    {
        ...
        Log::info("getViolations method called", [
        ...
        $violations = TasFile::whereDate("created_at", $date)
            ->latest()
            ->get();
    ...
    public function indexa(Request $request)
    {
        ...
        $revenueThisMonth = TasFile::whereMonth(
            "date_received",
            date("m")
        )->count();
        $previousMonthRevenue = TasFile::whereMonth(
            "date_received",
            Carbon::now()->subMonth()
        )->count();

        // Fetch recent activity (assumed to be last 5 records created today)
        $recentActivity = TasFile::whereDate("created_at", today())
            ->latest()
            ->take(5)
            ->get();

        // Count customers received this year
        $customersThisYear = TasFile::whereYear(
            "date_received",
            now()
        )->count();
        ...
        $averageSalesLastWeek =
            TasFile::whereBetween("created_at", [
                Carbon::now()
                    ->subDays(7)
                    ->startOfDay(),
                Carbon::now()
        ...
        $chartData = $admittedData->map(function ($item) {
            ...
            if ($item->violation) {
                ...
                $violationCount = is_array($violations)
                    ? count($violations)
                    : 0;
            }
            return [
        ...
        $officers = TasFile::leftJoin(
            "apprehending_officers",
            "tas_files.apprehending_officer",
            "=",
            "apprehending_officers.officer"
        )
            ->select(
                "tas_files.apprehending_officer",
                "apprehending_officers.department"
            )
        ...
        $departments = ApprehendingOfficer::select("department")
            ->distinct()
            ->get();
        ...
        $countByMonth = $allMonths->map(function ($month) use ($countByMonth) {
            return $countByMonth->has($month["month"])
                ? $countByMonth[$month["month"]]
                : $month;
        });
        ...
        $recentSalesTodayCaseAdmitted = admitted::whereDate(
            "created_at",
            today()
        )
            ->orderBy("created_at", "desc")
        ...
        return view(
            "index",
            compact(
                "recentSalesTodayTasFile",
                "recentSalesTodayCaseAdmitted",
                "departments",
                "officers",
                "yearlyData",
                "countByMonth",
    ...
    {
        $officers = ApprehendingOfficer::where(
            "department",
            $departmentName
        )->get();
    ...
    public function updateAdmittedCase(Request $request, $id)
    {
        ...
        return redirect()
            ->back()
            ->with("success", "Admitted case updated successfully");
    }
    ...
    public function reportsview(Request $request)
    {
        ...
        $tasFiles = TasFile::whereBetween("date_received", [
            $startDate,
            $endDate,
        ])->get();
        ...
        foreach ($tasFiles as $tasFile) {
            ...
            if ($violations) {
                if (is_array($violations)) {
                    $relatedViolations = TrafficViolation::whereIn(
                        "code",
                        $violations
                    )->get();
                } else {
                    $relatedViolations = TrafficViolation::where(
                        "code",
                        $violations
                    )->get();
                }
            ...
            $totalFinePerViolation = $totalFinePerViolation->merge(
                $relatedViolations->pluck("fine")
            );
    ...
    public function tasView()
    {
        try {
            ...
            foreach ($tasFiles as $tasFile) {
                ...
                $officersForFile = ApprehendingOfficer::where(
                    "officer",
                    $officerName
                )->get();
                ...
                } elseif (is_array($tasFile->remarks)) {
    ...
    public function admitview()
    {
        ...
        foreach ($admitted as $admit) {
            ...
            $officer = ApprehendingOfficer::firstOrCreate([
                "officer" => $officerName,
            ]);
    ...
    public function saveRemarks(Request $request)
    {
        ...
        try {
            ...
            $newRemark =
                $remarks . " - " . $timestamp . " - " . Auth::user()->fullname;
    ...
    public function submitForm(Request $request)
    {
        try {
            $validatedData = $request->validate([
                ...
                "status" =>
                    "nullable|string|in:closed,in-progress,settled,unsettled",
            ...
            $existingTasFile = TasFile::where(
                "case_no",
                $validatedData["case_no"]
            )->first();
            ...
            if (!$existingTasFile) {
                $tasFile = new TasFile([
                    ...
                    "apprehending_officer" =>
                        $validatedData["apprehending_officer"],
                    "violation" => json_encode(
                        explode(", ", $validatedData["violation"])
                    ),
                    "transaction_no" => $validatedData["transaction_no"]
                        ? "TRX-LETAS-" . $validatedData["transaction_no"]
                        : null,
                ...
                return redirect()
                    ->back()
                    ->with("error", "Case no. already exists.");
            }
    ...
    public function archivessubmit(Request $request)
    {
        try {
            $validatedData = $request->validate([
                ...
                "plate_no" => "required|string",
                "status" =>
                    "required|string|in:closed,in-progress,settled,unsettled",
                "file_attachment" => "nullable|array",
                "file_attachment.*" => "nullable|file|max:512000",
                "typeofvehicle" => "required|string", // Add validation for typeofvehicle
            ]);
            ...
            $existingarchive = archives::where(
                "tas_no",
                $validatedData["tas_no"]
            )->first();
            ...
            if (!$existingarchive) {
                ...
                return redirect()
                    ->back()
                    ->with("error", "tas no. already exists.");
            }
    ...
    public function archivesview()
    {
        ...
        foreach ($archives as $archive) {
            ...
            } elseif (is_array($archive->remarks)) {
    ...
    public function detailsarchives(Request $request, $id)
    {
        try {
            ...
            $relatedOfficers = ApprehendingOfficer::where(
                "officer",
                $archives->apprehending_officer
            )->get();
    ...
    public function detailarchivesedit(Request $request, $id)
    {
        try {
            ...
            return view(
                "case_archive.detailsedit",
                compact(
                    "recentViolationsToday",
                    "officers",
                    "violationz",
                    "remarks"
                )
    ...
    public function updatearchives()
    {
        ...
        return view(
            "case_archive.edit",
            compact("recentViolationsToday", "violation", "officers")
        );
    }
    ...
    public function updatedetailarchives(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile("file_attach_existing")) {
                ...
                $existingFilePaths =
                    json_decode($violation->file_attach, true) ?? [];
            ...
            // Process remarks
            if (
                isset($validatedData["remarks"]) &&
                is_array($validatedData["remarks"])
            ) {
                $remarksArray = [];
            ...
            if (!empty($validatedData["violation"])) {
                $existingViolations =
                    json_decode($violation->violation, true) ?? [];
                $newViolations = array_filter(
                    $validatedData["violation"],
                    function ($value) {
                        return $value !== null;
                    }
                );
                $validatedData["violation"] = json_encode(
                    array_unique(
                        array_merge($existingViolations, $newViolations)
                    )
                );
            }
            ...
            return response()->json(
                ["error" => "Error updating Violation: " . $e->getMessage()],
                500
            );
        }
    }
    public function removeAttachmentarchives(Request $request, $id)
    {
        try {
            ...
            if ($attachmentToRemove) {
                ...
                if (
                    ($key = array_search($attachmentToRemove, $attachments)) !==
                    false
                ) {
                    unset($attachments[$key]);
                    $archive->file_attach = json_encode(
                        array_values($attachments)
                    ); // Reindex array and encode back to JSON
                    $archive->save();
                    ...
                    return response()->json(
                        ["error" => "Attachment not found in the list."],
                        404
                    );
                }
            } else {
                return response()->json(
                    ["error" => "Attachment parameter is missing."],
                    400
                );
            }
        } catch (\Exception $e) {
    ...
    function delete_archives($id)
    {
        try {
            ...
            $violation->delete();
            return response()->json(
                ["success" => "Cases deleted successfully"],
                200
            );
        } catch (\Exception $e) {
            Log::error("Error deleting Violation: " . $e->getMessage());
            return response()->json(
                ["error" => "Error deleting Violation: " . $e->getMessage()],
                500
            );
        }
    }
    public function update_archive_violation(Request $request, $id)
    {
        ...
        return response()->json([
            ...
            "violations" => json_decode($archives->violation),
        ]);
    }
    ...
    public function admittedsubmit(Request $request)
    {
        ...
        try {
            ...
            DB::beginTransaction();
            $currentYear = date("Y");
            $existingadmitted = admitted::where(
                "admittedno",
                $validatedData["admittedno"]
            )->first();
            if (!$existingadmitted) {
                ...
                return redirect()
                    ->back()
                    ->with("error", "Admitted no. already exists.");
            }
            DB::commit();
            return redirect()
                ->back()
    ...
    public function update(Request $request, $id)
    {
        try {
            $request->validate([
                ...
                "profile_picture" =>
                    "nullable|image|mimes:jpeg,png,jpg,gif|max:6144",
            ]);
            $user = User::findOrFail($id);
            if ($request->hasFile("profile_picture")) {
                ...
                $filename =
                    Auth::user()->id .
                    "_" .
                    $profilePicture->getClientOriginalName();
    ...
    public function management()
    {
        ...
        return view("user.user_management", ["users" => $users]);
    }
    ...
    public function store_user(Request $request)
    {
        try {
            ...
            $encryptedPassword = Crypt::encryptString(
    ...
    public function save_offi(Request $request)
    {
        try {
            ...
            $existingOfficer = ApprehendingOfficer::where(
                "officer",
                $request->input("officer")
            )
            ...
            if ($existingOfficer) {
                return redirect()
                    ->back()
                    ->with("error", "Officer already exists.");
            }
    ...
    public function addvio(Request $request)
    {
        try {
            ...
            return redirect()
                ...
                ->with(
                    "error",
                    "Error creating Violation: " . $e->getMessage()
                );
        }
    }
    public function updateTas(Request $request, $id)
    {
        try {
            ...
            if (
                isset($validatedData["transaction_no"]) &&
                $validatedData["transaction_no"] !== $violation->transaction_no
            ) {
                // Remove any existing "TRX-LETAS-" prefix from the new transaction_no
                $validatedData["transaction_no"] = preg_replace(
                    "/^TRX-LETAS-/",
                    "",
                    $validatedData["transaction_no"]
                );
                // Ensure only one "TRX-LETAS-" prefix is added
                $validatedData["transaction_no"] =
                    "TRX-LETAS-" .
                    ltrim($validatedData["transaction_no"], "TRX-LETAS-");
            } else {
            ...
            return back()->with(
                "error",
                "Error updating Violation: " . $e->getMessage()
            );
        }
    }
    ...
    public function uploadFileAdmit(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile("file_attach")) {
                $existingFilePaths =
                    json_decode($admitted->file_attach, true) ?? [];
                $cx = count($existingFilePaths) + 1;
    ...
    public function uploadFileTas(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile("file_attach")) {
                ...
                // Retrieve existing file attachments and decode them
                $existingFilePaths =
                    json_decode($tasFile->file_attach, true) ?? [];
            ...
            return response()->json(
                ...
                500
            ); // You can adjust the status code if needed
        }
    }
    ...
    public function printsub(Request $request, $id)
    {
        ...
        while ($numDays > 0) {
            ...
            if (
                ...
                in_array($currentDate->format("m-d"), $holidays)
            ) {
    ...
    public function printsubadmit(Request $request, $id)
    {
        $admitted = Admitted::findOrFail($id);
        $changes = $admitted;
        $officerName = $changes->apprehending_officer;
        ...
        switch ($status) {
            case "subpeona":
                return view("sub.print", compact("admitted", "compactData"));
            case "motionrelease1":
                return view(
                    "sub.motionreleasep1",
                    compact("admitted", "compactData")
                );
            case "motionrelease2":
                return view(
                    "sub.motionreleasep2",
    ...
    public function updateAdmitted()
    {
        ...
        return view(
            "admitted.edit",
            compact("recentViolationsToday", "codes", "officers")
        );
    }
    ...
    public function updateoffi(Request $request, $id)
    {
        try {
            ...
            return back()->with(
                "error",
                "Failed to update officer details. Please try again."
            );
        }
    }
        ...
        try {
            ...
            if ($attachmentToRemove) {
                ...
                if (
                    ...
                    unset($attachments[$key]);
                    $tasFile->file_attach = json_encode(
                        array_values($attachments)
                    ); // Reindex array and encode back to JSON
                    $tasFile->save();
    ...
    public function DELETEVIO(Request $request, $id)
    {
        try {
            ...
            if (isset($violations[$violationIndex])) {
                ...
                \Log::info(
                    "Violation at index $violationIndex deleted successfully for TasFile ID: $id."
                );

                // Log the update of the completeness status
                \Log::info(
                    "TasFile ID: $id completeness checked and symbols updated."
                );
            } else {
                \Log::warning(
                    "Violation index $violationIndex not found for TasFile ID: $id."
                );
                return response()->json(
                    ["message" => "Violation index not found"],
                    404
                );
            }
    ...
    public function analyticsDash()
    {
        ...
        $violationsData = TasFile::select(
            DB::raw("violation, count(*) as count")
        )
            ->groupBy("violation")
            ->get();
        ...
        return view(
            "analytics",
            compact("chartData", "yearsWithData", "labels", "data")
        );
    }
    ...
    public function fetchMonthlyTypeOfVehicle(Request $request)
    {
        ...
        $selectedMonthData = TasFile::select(
            DB::raw("DATE_FORMAT(date_received, '%Y-%m') as month"),
            "typeofvehicle",
            DB::raw("COUNT(*) as total")
        )
            ->whereRaw("DATE_FORMAT(date_received, '%Y-%m') = ?", [
                $selectedMonth,
            ])
        ...
        $comparisonMonthData = TasFile::select(
            DB::raw("DATE_FORMAT(date_received, '%Y-%m') as month"),
            "typeofvehicle",
            DB::raw("COUNT(*) as total")
        )
    ...
    public function getDateReceivedData(Request $request)
    {
        try {
            ...
            $dateCounts = $data
                ->groupBy(function ($item) {
                    return $item->date_received instanceof \Carbon\Carbon
                        ...
                        : \Carbon\Carbon::parse($item->date_received)->format(
                            "Y-m-d"
                        );
                })
                ->map(function ($group) {
            ...
            \Log::error(
                "Error fetching date received data: " . $e->getMessage()
            );
    ...
    public function fetchViolations(Request $request)
    {
        ...
        if (
            ...
            return response()->json(
                ["error" => "Invalid months or years selected"],
                400
            );
        }
    ...
    public function getViolationRankings(Request $request)
    {
        ...
        foreach ($tasFiles as $tasFile) {
            ...
            preg_match_all(
                '/\"(.*?)\"|([A-Za-z0-9\s]+(?:\s+AND\s+[A-Za-z0-9\s]+)*)/',
                $tasFile->violation,
                $matches
            );
        ...
        $paginatedViolations = array_slice(
            $violationsCount,
            $offset,
            $perPage,
            true
        ...
        foreach ($paginatedViolations as $violationCode => $count) {
            $violation = TrafficViolation::where(
                "code",
                $violationCode
            )->first();
            if ($violation) {
    ...
    public function getPieChartData(Request $request)
    {
        try {
            ...
            $violationDetails = TrafficViolation::whereIn(
                "code",
                array_keys($violationCounts)
            )->get();
            ...
            foreach ($violationDetails as $violationDetail) {
                ...
                if (isset($violationCounts[$code])) {
                    ...
                    Log::warning(
                        "Violation code not found in counts: " . $code
                    );
                }
            }
    ...
    public function getChatData($userId)
    {
        try {
            ...
            while (true) {
                ...
                $messages = G5ChatMessage::where(function ($query) use (
                    $userId
                ) {
                    $query
                ...
                $messages->transform(function ($message) {
                    $message->created_at_formatted = Carbon::parse(
                        $message->created_at
                    )->format("M d, Y H:i A");
                    return $message;
                });
